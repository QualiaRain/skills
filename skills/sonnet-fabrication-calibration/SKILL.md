...
description: >-
  How to calibrate TRUST in a cheaper model's output (Sonnet, Haiku, or any model below your strongest) - cheap models confidently fabricate authoritative-looking specifics, a failure more effort does NOT fix. Use before relying on a cheaper model's answer when a load-bearing specific is at stake - a number, name, date, citation, API signature, config key. Also - can I trust this Sonnet/Haiku/subagent answer.
---

# Calibrating trust in cheaper models (confident fabrication)

Measured on Sonnet (and Haiku), with Opus as the reference, so the text says "Sonnet"; read it as **any cheaper model you route work to**. The measurements are the author's, on the model versions of the time.
...
## The validated finding (the author's eval, 2026-06-13; harness not shipped with this pack)
...
2. **Tool-verify it.** Run the code, hit the API, grep the file, do the date math in a script. Deterministic, zero hallucination risk, ~free. Prefer this for anything mechanically checkable.
...
4. **Escalate the MODEL, not the effort.** If a specific is load-bearing and cannot be anchored or tool-checked, send *that piece* to your strongest model (Opus was flawless in the measured run) — not "Sonnet at higher effort," which does not help (effort is not the lever). Keep the cheap model for the surrounding work; spend the strong model narrowly, on the specific.

**Done check:** every gate-list specific in the output you are about to rely on has been anchored, tool-verified, cross-checked, or escalated — or is explicitly flagged to the user as unverified.
...
- **Picking the verifier's model** for a gate → `phase-gated-model-routing` (its escalation-ladder eval section). **Running a self-verifying loop** (writer ≠ grader is the same principle) → `loop-engineering`.

## Maintenance — keep the calibration current

If you rely on this calibration, maintain it:
- **The finding is dated to Sonnet 4.6 (2026-06-13) and is model-baseline behavior, not a config artifact** — so it holds until the model changes. **Recheck trigger: when the cheap model you route to changes version, re-run an eval before trusting this calibration** — fabrication rate and shape can shift with a new model. Update this skill to the new receipts; never carry a stale rate.
- **Re-validate** with a small eval of your own (the author's harness is not included): ~8 everyday paste-and-ask tasks with known answers, a blind judge on your strongest model, the cheap model ×3 trials per task for consistency. Make it default to a no-cost dry run so a mistaken launch costs nothing. Re-confirm the two load-bearing claims: (a) the specifics-fabrication still reproduces, (b) effort is still not the lever.
- **Harden this skill's execution and triggering** with the weak-model loop (`skill-execution-hardening`): does a cold Haiku session, handed a specific-bearing task plus this skill, actually insert the gate? Writer ≠ grader — a fresh subagent grades, never the author.
